//! Bash command validation for LiteDuck's terminal safety layer.
//!
//! Self-contained module (no crate-internal dependencies) that ports the
//! upstream `BashTool` validation pipeline:
...
//! - `modeValidation`          — enforce permission level constraints
...
/// Permission level granted to the bash executor.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum PermissionLevel {
    /// No writes permitted; only read operations are allowed.
    ReadOnly,
    /// Writes scoped to the current workspace directory.
    WorkspaceWrite,
    /// Unrestricted — all commands are permitted.
    FullAccess,
}

/// Outcome of validating a bash command before execution.
#[derive(Debug, Clone, PartialEq, Eq, Serialize, Deserialize)]
pub enum ValidationResult {
    /// Command is safe to execute.
    Allow,
    /// Command must be blocked; `reason` explains why.
    Block { reason: String },
    /// Command requires user confirmation; `message` explains the concern.
    Warn { message: String },
}
...
pub enum CommandIntent {
    ...
    ReadOnly,
    /// Filesystem writes: cp, mv, mkdir, touch, tee, etc.
    Write,
...
// ---------------------------------------------------------------------------
// Constants — readOnlyValidation
// ---------------------------------------------------------------------------

/// Commands that perform write operations and must be blocked in read-only mode.
const WRITE_COMMANDS: &[&str] = &[
    "cp", "mv", "rm", "mkdir", "rmdir", "touch", "chmod", "chown", "chgrp", "ln", "install", "tee",
    "truncate", "shred", "mkfifo", "mknod", "dd",
];

/// Commands that modify system state and must be blocked in read-only mode.
...
// ---------------------------------------------------------------------------
// Constants — destructiveCommandWarning
// ---------------------------------------------------------------------------

/// Patterns that indicate potentially destructive commands (pattern, warning).
...
/// Steps: mode → sed → destructive → paths.
#[must_use]
pub fn validate_command(
    ...
    let result = validate_mode(command, level.clone());
    if result != ValidationResult::Allow {
        return result;
    }

    // 2. Sed-specific validation.
    let result = validate_sed(command, level.clone());
    if result != ValidationResult::Allow {
        return result;
    }
...
// ---------------------------------------------------------------------------
// Public API — individual validators
// ---------------------------------------------------------------------------

/// Validate that a command is allowed under the current permission level.
///
/// Blocks write commands, state-modifying commands, write redirections, and
/// non-read-only git subcommands when `level` is `ReadOnly`.
/// Returns `Allow` for all other levels.
#[must_use]
pub fn validate_read_only(command: &str, level: PermissionLevel) -> ValidationResult {
    ...
    if first_command == "sudo" {
        ...
        if !inner.is_empty() {
            let inner_result = validate_read_only(inner, level);
...
///
/// Checks known destructive patterns and always-destructive commands.
#[must_use]
...
/// Validate that a command is consistent with the given permission level.
///
/// - `ReadOnly`: delegates to `validate_read_only`.
/// - `WorkspaceWrite`: warns when the command targets system paths outside the workspace.
/// - `FullAccess`: always allows.
#[must_use]
pub fn validate_mode(command: &str, level: PermissionLevel) -> ValidationResult {
    match level {
        PermissionLevel::ReadOnly => validate_read_only(command, level),
        ...
        PermissionLevel::FullAccess => ValidationResult::Allow,
    }
}

/// Validate sed expressions for safety.
///
/// Blocks `sed -i` (in-place editing) in read-only mode.
#[must_use]
pub fn validate_sed(command: &str, level: PermissionLevel) -> ValidationResult {
    let first = extract_first_command(command);
    if first != "sed" {
        return ValidationResult::Allow;
    }

    if level == PermissionLevel::ReadOnly && command.contains(" -i") {
        return ValidationResult::Block {
...
///
/// Warns on `../` traversal (when the workspace root is not mentioned) and on
/// home-directory references (`~/` or `$HOME`).
#[must_use]
...
/// Extract the command string following "sudo" (skips sudo flags).
...
/// Check whether a `git` command is read-only; block non-read-only subcommands.
...
/// Dispatch classification based on the first command token.
fn classify_by_first_command(first: &str, command: &str) -> CommandIntent {
    if SEMANTIC_READ_ONLY_COMMANDS.contains(&first) {
        // sed -i is a write even though sed is normally read-only.
...
mod tests {
    ...
    fn blocks_rm_in_read_only() {
        assert!(matches!(
            validate_read_only("rm -rf /tmp/x", PermissionLevel::ReadOnly),
    ...
    fn allows_rm_in_workspace_write() {
        assert_eq!(
            validate_read_only("rm -rf /tmp/x", PermissionLevel::WorkspaceWrite),
            ValidationResult::Allow
        );
    }
    ...
    fn blocks_write_redirections_in_read_only() {
        assert!(matches!(
            validate_read_only("echo hello > file.txt", PermissionLevel::ReadOnly),
    ...
    fn allows_read_commands_in_read_only() {
        assert_eq!(
            validate_read_only("ls -la", PermissionLevel::ReadOnly),
            ValidationResult::Allow
        );
        assert_eq!(
            validate_read_only("cat /etc/hosts", PermissionLevel::ReadOnly),
            ValidationResult::Allow
        );
        assert_eq!(
            validate_read_only("grep -r pattern .", PermissionLevel::ReadOnly),
            ValidationResult::Allow
        );
    }
    ...
    fn blocks_sudo_write_in_read_only() {
        assert!(matches!(
            validate_read_only("sudo rm -rf /tmp/x", PermissionLevel::ReadOnly),
            ValidationResult::Block { reason } if reason.contains("rm")
        ));
    }
    ...
    fn blocks_git_push_in_read_only() {
        assert!(matches!(
            validate_read_only("git push origin main", PermissionLevel::ReadOnly),
    ...
    fn allows_git_status_in_read_only() {
        assert_eq!(
            validate_read_only("git status", PermissionLevel::ReadOnly),
            ValidationResult::Allow
        );
    }
    ...
    fn blocks_package_install_in_read_only() {
        assert!(matches!(
            validate_read_only("npm install express", PermissionLevel::ReadOnly),
    ...
    fn workspace_write_warns_system_paths() {
        assert!(matches!(
            validate_mode("cp file.txt /etc/config", PermissionLevel::WorkspaceWrite),
    ...
    fn workspace_write_allows_local_writes() {
        assert_eq!(
            validate_mode("cp file.txt ./backup/", PermissionLevel::WorkspaceWrite),
            ValidationResult::Allow
        );
    }
    ...
    fn blocks_sed_inplace_in_read_only() {
        assert!(matches!(
            validate_sed("sed -i 's/old/new/' file.txt", PermissionLevel::ReadOnly),
    ...
    fn allows_sed_stdout_in_read_only() {
        assert_eq!(
            validate_sed("sed 's/old/new/' file.txt", PermissionLevel::ReadOnly),
            ValidationResult::Allow
        );
    }
    ...
    fn pipeline_blocks_write_in_read_only() {
        ...
        assert!(matches!(
            validate_command("rm -rf /tmp/x", PermissionLevel::ReadOnly, &workspace),
    ...
    fn pipeline_warns_destructive_in_write_mode() {
        let workspace = PathBuf::from("/workspace");
        assert!(matches!(
            validate_command("rm -rf /", PermissionLevel::WorkspaceWrite, &workspace),
    ...
    fn pipeline_allows_safe_read_in_read_only() {
        let workspace = PathBuf::from("/workspace");
        assert_eq!(
            validate_command("ls -la", PermissionLevel::ReadOnly, &workspace),
            ValidationResult::Allow
        );
    }
    ...
    #[test]
    fn allows_full_access_mode_everything() {
        // Even a write command is allowed at the validate_mode level in FullAccess.
        assert_eq!(
            validate_mode("rm -rf /", PermissionLevel::FullAccess),
            ValidationResult::Allow
        );
        assert_eq!(
            validate_read_only("rm -rf /", PermissionLevel::FullAccess),
            ValidationResult::Allow
        );
    }
    ...
    #[test]
    fn blocks_state_modifying_in_read_only() {
        assert!(matches!(
            validate_read_only("apt install curl", PermissionLevel::ReadOnly),
            ValidationResult::Block { reason } if reason.contains("apt")
        ));
        assert!(matches!(
            validate_read_only("brew install jq", PermissionLevel::ReadOnly),
            ValidationResult::Block { reason } if reason.contains("brew")
        ));
        assert!(matches!(
            validate_read_only("docker pull ubuntu", PermissionLevel::ReadOnly),
            ValidationResult::Block { reason } if reason.contains("docker")
        ));
    }
}
